...
- (void)ekalgorithms {
    NSMutableArray *array = [self.testArray mutableCopy];
    ...
    [EKTree forestToBinaryTree:@[forest1, forest2]];
}
- (void)addCaseButton {
    WS(ws);
    ...
    [btn4 setTitle:@"Binary search" forState:UIControlStateNormal];
...
- (void)testMethod:(UIButton *)btn {
    ...
    switch (btn.tag) {
        ...
        case 2:
        {
            ...
            [self.testArray enumerateObjectsWithOptions:NSEnumerationConcurrent usingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
                if ([obj isEqualToString:@"xxx"]) {
                    NSLog(@"%@",obj);
                }
            }];
            break;
        ...
        {
            NSLog(@"********Linear search");
            [self.testArray indexOfObjectViaLinearSearch:@"999"];
            break;
        }
        case 4:
        {
            NSLog(@"********Binary search");
            [self.testArray indexOfObjectViaBinarySearch:@"999"];
            break;
        }
    }
    TOCK;
